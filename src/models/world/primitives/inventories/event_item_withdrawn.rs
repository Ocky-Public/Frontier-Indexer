...
use crate::schema::events_item_withdrawn;

#[derive(Deserialize)]
pub struct MoveItemWithdrawn {
...
#[derive(Deserialize)]
pub struct MoveItemWithdrawnV2 {
    pub assembly_id: Address,
    pub assembly_key: MoveTenantItemId,
...
impl StoredItemWithdrawn {
    pub fn from_event(event: &Event, meta: &EventMeta) -> Self {
        let move_event: MoveItemWithdrawn =
            bcs::from_bytes(&event.contents).expect("Failed to deserialize Item Withdrawn event");
    ...
    pub fn from_event_v2(event: &Event, meta: &EventMeta) -> Self {
        let move_event: MoveItemWithdrawnV2 = bcs::from_bytes(&event.contents)
            .expect("Failed to deserialize Item Withdrawn V2 event");
