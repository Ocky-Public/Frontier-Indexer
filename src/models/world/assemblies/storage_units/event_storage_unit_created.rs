...
use crate::schema::indexer::events_storage_unit_created;

#[derive(Deserialize)]
pub struct MoveStorageUnitCreated {
    pub storage_unit_id: Address,
...
#[diesel(table_name = events_storage_unit_created)]
#[diesel(check_for_backend(diesel::pg::Pg))]
pub struct StoredStorageUnitCreated {
    pub event_id: String,
...
impl StoredStorageUnitCreated {
    pub fn from_event(event: &Event, meta: &EventMeta) -> Self {
        let move_event: MoveStorageUnitCreated =
            bcs::from_bytes(&event.contents).expect("Failed to deserialize Storage Unit Created event");
        ...
        Self {
            ...
            occurred_at,
            id: move_event.storage_unit_id.to_hex(),
