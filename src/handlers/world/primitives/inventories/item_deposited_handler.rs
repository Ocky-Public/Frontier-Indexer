...
pub struct ItemDepositedHandler {
    ctx: AppContext,
    emitter: Arc<Emitter<StoredItemDeposited>>,
}

impl ItemDepositedHandler {
    ...
    fn is_item_deposited(&self, event: &Event) -> bool {
        ...
        let event_name = "ItemDepositedEvent";
        self.ctx.is_world_event(event, module_name, event_name)
    }
}

#[async_trait]
impl Processor for ItemDepositedHandler {
    const NAME: &'static str = "item_deposited";
        ...
        for tx in &checkpoint.transactions {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                if self.is_item_deposited(ev) {
                    ...
                    let event = StoredItemDeposited::from_event(ev, &meta);
                    results.push(event);
                }
            }
        }
...
#[async_trait]
impl Handler for ItemDepositedHandler {
    type Store = Db;
    ...
    async fn commit<'a>(
        ...
        use crate::schema::indexer::events_item_deposited::dsl::*;
