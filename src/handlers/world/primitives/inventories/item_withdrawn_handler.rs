...
pub struct ItemWithdrawnHandler {
  ctx: AppContext,
}

impl ItemWithdrawnHandler {
    ...
    fn is_item_burned(&self, event: &Event) -> bool {
        ...
        let event_name = "ItemWithdrawnEvent";
        self.ctx.is_world_event(event, module_name, event_name)
    }
}

#[async_trait]
impl Processor for ItemWithdrawnHandler {
    const NAME: &'static str = "item_withdrawn";
        ...
        for tx in &checkpoint.transactions {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                if self.is_item_burned(ev) {
                    ...
                    let event = StoredItemWithdrawn::from_event(ev, &meta);
                    results.push(event);
                }
            }
        }
...
#[async_trait]
impl Handler for ItemWithdrawnHandler {
    type Store = Db;
    ...
    async fn commit<'a>(
        ...
        use crate::schema::indexer::events_item_withdrawn::dsl::*;
