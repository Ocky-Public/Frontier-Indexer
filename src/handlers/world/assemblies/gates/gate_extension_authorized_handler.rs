...
#[async_trait]
impl Processor for GateExtensionAuthorizedHandler {
    const NAME: &'static str = "gate_extension_authorized";
    type Value = StoredGateExtensionAuthorized;
    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
        ...
        for tx in &checkpoint.transactions {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                if self.is_gate_extension_authorized(ev) {
                    let meta = base_meta.with_index(index);
                    let event = StoredGateExtensionAuthorized::from_event(ev, &meta);
                    results.push(event);
                }
            }
        }
...
#[async_trait]
impl Handler for GateExtensionAuthorizedHandler {
    type Store = Db;
    ...
    async fn commit<'a>(
        ...
        use crate::schema::indexer::events_gate_extension_authorized::dsl::*;

        diesel::insert_into(events_gate_extension_authorized)
            .values(batch)
