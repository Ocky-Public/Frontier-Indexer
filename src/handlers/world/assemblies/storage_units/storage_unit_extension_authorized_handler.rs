...
use crate::models::world::StoredStorageUnitExtensionAuthorized;
use crate::transports::Transport;

use crate::AppContext;

pub struct StorageUnitExtensionAuthorizedHandler {
    ctx: AppContext,
    emitter: Arc<Emitter<StoredStorageUnitExtensionAuthorized>>,
}

impl StorageUnitExtensionAuthorizedHandler {
    pub fn new(
        ctx: &AppContext,
        transports: Vec<Arc<dyn Transport<StoredStorageUnitExtensionAuthorized>>>,
    ) -> Self {
    ...
    fn is_storage_unit_extension_authorized(&self, event: &Event) -> bool {
...
#[async_trait]
impl Processor for StorageUnitExtensionAuthorizedHandler {
    const NAME: &'static str = "storage_unit_extension_authorized";
    type Value = StoredStorageUnitExtensionAuthorized;
    ...
    async fn process(&self, checkpoint: &Arc<Checkpoint>) -> anyhow::Result<Vec<Self::Value>> {
        ...
        for tx in &checkpoint.transactions {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                if self.is_storage_unit_extension_authorized(ev) {
                    let meta = base_meta.with_index(index);
                    let event = StoredStorageUnitExtensionAuthorized::from_event(ev, &meta);
                    results.push(event);
                }
            }
        }
...
#[async_trait]
impl Handler for StorageUnitExtensionAuthorizedHandler {
    type Store = Db;
